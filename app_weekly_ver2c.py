...
def load_data(ticker):
    ...
    # Feature Engineering: Calculate weekly exponential moving averages
    weekly_data['EMA5'] = weekly_data['Close'].ewm(span=5, adjust=False).mean()
    weekly_data['EMA10'] = weekly_data['Close'].ewm(span=10, adjust=False).mean()
    weekly_data['EMA20'] = weekly_data['Close'].ewm(span=20, adjust=False).mean()
...
if ticker:
    try:
        ...
        # Feature Engineering
        features = ['Open', 'High', 'Low', 'Close', 'Volume', 'EMA5', 'EMA10', 'EMA20', 'RSI']
        X = data[features]
        y = data['Target']
